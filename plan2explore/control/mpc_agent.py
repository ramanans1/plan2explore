...
from plan2explore.tools import nested
...
class MPCAgent(object):
  ...
  def step(self, agent_indices, observ):
    ...
    if self._adaptation and self._config.prefix=='train':
        action = tf.cond(tf.logical_and(tf.equal(self._phase, 'train'),
            self._globalstep < tf.cast(self._config.adaptation_step, tf.int64)),
            lambda: self._config.planner(
            self._cell, self._config.objective, state,
            embedded.shape[1:].as_list(),
            prev_action.shape[1:].as_list()),
            lambda: self._config.secondary_planner(
            self._cell, self._config.secondary_objective, state,
            embedded.shape[1:].as_list(),
            prev_action.shape[1:].as_list()))
